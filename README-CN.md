...
不过目前已经发布的桌面 Release 版本还不算稳定，在不同机器上可能会有兼容性问题。现阶段如果你想稳定使用，优先推荐直接走源码下的 CLI。
...
如果桌面 Release 暂时在你的机器上跑不起来，推荐直接使用 CLI。

macOS：

```bash
cd /path/to/Folio
cargo run -p scribe-cli -- "/绝对路径/输入.md" -o "/绝对路径/输出.docx"
```
...
```powershell
cd C:\path\to\Folio
cargo run -p scribe-cli -- "C:\绝对路径\输入.md" -o "C:\绝对路径\输出.docx"
```

如果省略 `-o`，默认会在源 Markdown 同目录生成同名 `.docx` 文件。

### Python（预览）

Folio 也以 Python wheel 形式发布，方便在脚本、Notebook、流水线里直接调用。
...
# Markdown 字符串 → .docx 字节
docx_bytes = folio.convert("# 你好\n\n$E = mc^2$")

# Markdown 文件 → .docx 文件（图片相对路径相对于输入文件目录解析）
folio.convert_file("paper.md", "paper.docx")

# 实时 HTML 预览
html = folio.preview_html("# 标题\n\n正文")
```
